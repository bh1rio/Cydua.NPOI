...
using Cydua.NPOI.HSSF.Record.PivotTable;
    ...
    public class RecordOrderer
    {
        ...
        private static int FindInsertPosForNewCondFormatTable(List<RecordBase> records)
        {
            ...
            { // -2 to skip EOF record
                ...
                Cydua.NPOI.HSSF.Record.Record rec = (Cydua.NPOI.HSSF.Record.Record)rb;
                switch (rec.Sid)
                {
        ...
        private static int FindDataValidationTableInsertPos(List<RecordBase> records)
        {
            ...
            while (i > 0)
            {
                ...
                {
                    Cydua.NPOI.HSSF.Record.Record nextRec = (Cydua.NPOI.HSSF.Record.Record)records[i + 1];
        ...
        private static bool IsDVTPriorRecord(RecordBase rb)
        {
            ...
            short sid = ((Cydua.NPOI.HSSF.Record.Record)rb).Sid;
            switch (sid)
            {
                case WindowTwoRecord.sid:
                case SCLRecord.sid:
